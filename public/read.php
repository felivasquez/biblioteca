...
<body>
    ...
    $sql = "SELECT libro_id, titulo,  FROM usuarios";
    ...
    if ($resultado->num_rows > 0) {
        ...
        while ($row = $resultado->fetch_assoc()) {
            echo "<div>";
            echo "ID: " . $row["libro_id"] . " - Nombre Usuario: " . $row["nombre"] . " " . $row["apellido"];
            // Formulario para actualizar el nombre y el apellido
            echo "<form method='POST' action='actualizar.php'>";
            echo "<input type='hidden' name='id_lista' value='" . $row["libro_id"] . "'>";
